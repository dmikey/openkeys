...
async function connectMidi() { if (!navigator.requestMIDIAccess) { showToast("Web MIDI is not available here. Use microphone mode or a supported desktop browser."); setInputMode("mic"); return; } try { midiAccess ||= await navigator.requestMIDIAccess(); for (const input of midiAccess.inputs.values()) input.onmidimessage = handleMidiMessage; midiAccess.onstatechange = () => { for (const input of midiAccess.inputs.values()) input.onmidimessage = handleMidiMessage; els.inputStatus.textContent = midiAccess.inputs.size ? "MIDI CONNECTED" : "CONNECT A KEYBOARD"; }; els.inputStatus.textContent = midiAccess.inputs.size ? "MIDI CONNECTED" : "CONNECT A KEYBOARD"; els.statusTitle.textContent = midiAccess.inputs.size ? "MIDI ready" : "Waiting for MIDI keyboard"; els.statusCopy.textContent = "Play the highlighted notes together"; } catch (_) { showToast("MIDI permission was not granted."); setInputMode("mic"); } }
function setInputMode(mode) { activeInput = mode; els.inputButtons.forEach(button => { const active = button.dataset.input === mode; button.classList.toggle("active",active); button.setAttribute("aria-pressed",active); }); if (mode === "midi") { stopListening(); els.mic.disabled = true; els.mic.querySelector("strong").textContent = "MIDI input active"; els.mic.querySelector("small").textContent = "Use your connected keyboard"; connectMidi(); } else { els.mic.disabled = false; els.mic.querySelector("strong").textContent = listening ? "Listening now" : "Start listening"; els.mic.querySelector("small").textContent = listening ? "Tap to stop" : "Uses your device microphone"; els.inputStatus.textContent = "ACOUSTIC MODE"; } }
